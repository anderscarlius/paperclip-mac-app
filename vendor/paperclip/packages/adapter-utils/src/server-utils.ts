...
function normalizePaperclipWakeIssue(value: unknown): PaperclipWakeIssue | null {
  ...
  const modeRaw = asString(issue.mode, "").trim().toLowerCase();
  const mode = modeRaw === "fast" || modeRaw === "deep" ? modeRaw : "normal";
  if (!id && !identifier && !title && !description) return null;
  return {
    id,
    identifier,
    title,
...
export function renderPaperclipWakePrompt(
  ...
  const issueText = [
    ...
    ...normalized.comments.map((comment) => comment.body),
  ].filter(Boolean).join("\n");
  const modeMatch = issueText.match(/\b(?:mode|effort)\s*:\s*(fast|normal|deep)\b/i)
    ?? issueText.match(/\[(fast|normal|deep)\]/i);
  const issueMode =
    normalized.issue?.mode ??
    (modeMatch?.[1]?.toLowerCase() as "fast" | "normal" | "deep" | undefined) ??
    "normal";
  ...
  const lines = resumedSession
      ? [
        ...
        "Before exploring the repo or scanning the workspace, checkout this issue and read its Paperclip issue context.",
        "Do not run broad repo discovery commands unless the issue context shows that code or local files are actually needed.",
        "If the issue can be answered directly from the issue title, description, or new comments, reply briefly and stop.",
        "For simple issue questions, do not use planning tools and do not attempt skill-execution tool calls.",
        "If live web information is needed, run the `paperclip-web-search \"query\" --max-results 5` shell command; never invent or call `skill_execute`, `process_skill`, `execute_skill`, or `exec_skill` tool calls.",
        `Issue mode: ${issueMode}. Fast means answer or act with minimal exploration; normal means inspect only what is clearly needed; deep means perform a careful implementation/research pass.`,
        "",
