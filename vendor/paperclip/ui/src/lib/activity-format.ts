...
function formatIssueUpdatedVerb(details: ActivityDetails): string | null {
  ...
  if (details.mode !== undefined) {
    const from = previous.mode;
    return from
      ? `changed mode from ${humanizeValue(from)} to ${humanizeValue(details.mode)} on`
      : `changed mode to ${humanizeValue(details.mode)} on`;
  }
  return null;
}
...
function formatIssueUpdatedAction(details: ActivityDetails): string | null {
  ...
  if (details.mode !== undefined) {
    ...
    parts.push(
      from
        ? `changed the mode from ${humanizeValue(from)} to ${humanizeValue(details.mode)}`
        : `changed the mode to ${humanizeValue(details.mode)}`,
    );
  }
