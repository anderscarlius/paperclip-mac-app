...
function isProductHiddenAdapterType(type: string): boolean {
  return type === "opencode_local";
}
...
export function listAdapterOptions(
  ...
  return adapters
    .filter((adapter) => !isProductHiddenAdapterType(adapter.type))
    .map((adapter) => ({
      value: adapter.type,
...
export function listVisibleUIAdapters(): UIAdapterModule[] {
  return listUIAdapters().filter((a) => !isAdapterTypeHidden(a.type) && !isProductHiddenAdapterType(a.type));
}
