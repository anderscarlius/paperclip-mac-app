...
import type { Issue, IssueMode } from "@paperclipai/shared";
...
function IssueModeSegmentedControl({
  value,
  onChange,
}: {
  value: IssueMode;
  onChange: (value: IssueMode) => void;
}) {
  return (
    <div className="inline-flex h-7 items-center rounded-md border border-border bg-muted/20 p-0.5">
      {ISSUE_MODE_OPTIONS.map((option) => (
        <button
          key={option.value}
          type="button"
          className={cn(
            "h-6 min-w-14 rounded px-2 text-xs font-medium transition-colors",
            value === option.value
    ...
    <div className="space-y-4">
      <div className="space-y-1">
        ...
        <PropertyRow label="Mode">
          <IssueModeSegmentedControl
            value={issue.mode ?? "normal"}
