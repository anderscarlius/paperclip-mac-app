...
describe("codex local adapter skill injection", () => {
  ...
  const webSearchKey = "paperclipai/paperclip/paperclip-web-search";
  ...
  it("removes command-bridged web-search skill symlinks instead of injecting them", async () => {
    const currentRepo = await makeTempDir("paperclip-codex-current-");
    const skillsHome = await makeTempDir("paperclip-codex-home-");
    cleanupDirs.add(currentRepo);
    cleanupDirs.add(skillsHome);

    await createPaperclipRepoSkill(currentRepo, "paperclip-web-search");
    await fs.symlink(
      path.join(currentRepo, "skills", "paperclip-web-search"),
      path.join(skillsHome, "paperclip-web-search"),
    );
    ...
    await ensureCodexSkillsInjected(
      ...
      },
      {
        ...
        skillsEntries: [{
          key: webSearchKey,
          runtimeName: "paperclip-web-search",
          source: path.join(currentRepo, "skills", "paperclip-web-search"),
        }],
      },
    );

    await expect(fs.lstat(path.join(skillsHome, "paperclip-web-search"))).rejects.toMatchObject({
      code: "ENOENT",
    });
    expect(logs).toContainEqual(
      expect.objectContaining({
