...
describe("codex local skill sync", () => {
  ...
  it("reports configured Paperclip skills for workspace injection on the next run", async () => {
    ...
    const webSearch = before.entries.find((entry) => entry.key === webSearchKey);
    expect(webSearch?.required).toBe(true);
    expect(webSearch?.state).toBe("configured");
    expect(webSearch?.targetPath).toBeNull();
    expect(webSearch?.detail).toContain("shell command");
    expect(webSearch?.detail).not.toContain("CODEX_HOME/skills/");
  });
