...
import {
  ...
  hasCodexThreadPersistenceFailureInResult,
  mergeCoalescedContextSnapshot,
  prioritizeProjectWorkspaceCandidatesForRun,
  parseSessionCompactionPolicy,
  resolveRuntimeSessionParamsForWorkspace,
  sanitizeAdapterResultForCodexThreadPersistenceFailure,
...
describe("Codex thread persistence fallback", () => {
  it("detects Codex thread persistence failures from adapter stderr", () => {
    expect(
      hasCodexThreadPersistenceFailureInResult({
        exitCode: 0,
        signal: null,
        ...
        resultJson: {
          stderr:
            "2026-04-29T14:03:40.304775Z ERROR codex_core::session: failed to record rollout items: thread 019dd98a-1ad5-74e3-aaa9-7f92a644c0de not found\n",
        },
      }),
    ).toBe(true);
  });

  it("sanitizes broken Codex sessions before heartbeat persistence", () => {
    const result = sanitizeAdapterResultForCodexThreadPersistenceFailure({
      exitCode: 0,
      signal: null,
      timedOut: false,
      errorMessage: null,
      clearSession: false,
      sessionId: "019dd98a-1ad5-74e3-aaa9-7f92a644c0de",
      sessionDisplayId: "019dd98a-1ad5-74e3-aaa9-7f92a644c0de",
      sessionParams: {
        sessionId: "019dd98a-1ad5-74e3-aaa9-7f92a644c0de",
      },
      resultJson: {
        stderr: [
          "2026-04-29T14:00:00.660122Z  WARN codex_core_plugins::manifest: ignoring interface.defaultPrompt: prompt must be at most 128 characters path=/tmp/plugin.json",
          "2026-04-29T14:03:40.304775Z ERROR codex_core::session: failed to record rollout items: thread 019dd98a-1ad5-74e3-aaa9-7f92a644c0de not found",
        ].join("\n"),
      },
    });
    ...
    expect((result.resultJson as Record<string, unknown>).stderr).not.toContain("failed to record rollout items");
    expect((result.resultJson as Record<string, unknown>).stderr).toContain("ignoring interface.defaultPrompt");
  });
});
