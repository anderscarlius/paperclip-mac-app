...
import { ISSUE_MODES, type BillingType, type ExecutionWorkspace, type ExecutionWorkspaceConfig, type IssueMode } from "@paperclipai/shared";
import {
  agents,
  agentRuntimeState,
  agentTaskSessions,
...
const MAX_INLINE_WAKE_ISSUE_DESCRIPTION_CHARS = 600;
...
const ISSUE_MODE_SET = new Set<string>(ISSUE_MODES);
...
export function hasCodexThreadPersistenceFailureInResult(result: AdapterExecutionResult): boolean {
  const resultJson = parseObject(result.resultJson);
  const stderr = readNonEmptyString(resultJson.stderr);
...
export function sanitizeAdapterResultForCodexThreadPersistenceFailure(
  result: AdapterExecutionResult,
): AdapterExecutionResult {
  const resultJson = parseObject(result.resultJson);
  const nextResultJson = Object.keys(resultJson).length > 0 ? { ...resultJson } : null;
  const stderr = readNonEmptyString(resultJson.stderr);
  if (stderr != null && nextResultJson) {
    const cleaned = stripCodexThreadPersistenceFailure(stderr).trim();
    if (cleaned) nextResultJson.stderr = cleaned;
    else delete nextResultJson.stderr;
  }
...
function enrichWakeContextSnapshot(input: {
  ...
  const payloadIssue = parseObject(payload?.["issue"]);
  const issueModeFromPayload = normalizeIssueMode(
    payload?.["issueMode"] ?? payloadIssue.mode ?? contextSnapshot.issueMode,
  );
  ...
  contextSnapshot.issueMode = issueModeFromPayload;
    ...
    issue: issueSummary
      ? {
          ...
          description: issueSummary.description
            ? issueSummary.description.slice(0, MAX_INLINE_WAKE_ISSUE_DESCRIPTION_CHARS)
            : null,
          status: issueSummary.status,
          priority: issueSummary.priority,
          mode: normalizeIssueMode(issueSummary.mode),
        }
      : null,
...
function normalizeIssueMode(value: unknown): IssueMode {
  return typeof value === "string" && ISSUE_MODE_SET.has(value) ? value as IssueMode : "normal";
}

async function resolveIssueMode(dbOrTx: Pick<Db, "select">, companyId: string, issueId: string | null | undefined) {
  if (!issueId) return "normal" as IssueMode;
  const row = await dbOrTx
    .select({ mode: issues.mode })
    .from(issues)
    .where(and(eq(issues.id, issueId), eq(issues.companyId, companyId)))
    .then((rows) => rows[0] ?? null);
  return normalizeIssueMode(row?.mode);
}
...
export function heartbeatService(db: Db) {
  ...
  async function enqueueMissingIssueCommentRetry(
    ...
    const retryContextSnapshot = {
      ...
      issueMode: normalizeIssueMode(contextSnapshot.issueMode ?? run.issueMode),
    };
    const retryIssueMode = retryContextSnapshot.issueMode;
    const now = new Date();
  ...
  async function executeRun(runId: string) {
    ...
    const issueContext = issueId
      ? await db
          ...
          .then((rows) => rows[0] ?? null)
      : null;
    const runIssueMode = normalizeIssueMode(issueContext?.mode ?? run.issueMode ?? context.issueMode);
    context.issueMode = runIssueMode;
    ...
    try {
      ...
      const adapterReportedThreadPersistenceFailure =
        hasCodexThreadPersistenceFailureInResult(adapterResult);
      const effectiveAdapterResult = adapterReportedThreadPersistenceFailure
        ? sanitizeAdapterResultForCodexThreadPersistenceFailure(adapterResult)
        : adapterResult;
      if (adapterReportedThreadPersistenceFailure && !stdoutExcerpt.includes("session thread was not persisted correctly")) {
        await onLog("stdout", paperclipThreadPersistenceWarningLine());
      }
      if (adapterReportedThreadPersistenceFailure) {
        stderrExcerpt = stripCodexThreadPersistenceFailure(stderrExcerpt).trim();
      }

      const nextSessionState = resolveNextSessionState({
        codec: sessionCodec,
        adapterResult: effectiveAdapterResult,
        previousParams: previousSessionParams,
        previousDisplayId: runtimeForAdapter.sessionDisplayId,
        previousLegacySessionId: runtimeForAdapter.sessionId,
      });
      const rawUsage = normalizeUsageTotals(effectiveAdapterResult.usage);
      ...
      if (latestRun?.status === "cancelled") {
        outcome = "cancelled";
      } else if (effectiveAdapterResult.timedOut) {
        outcome = "timed_out";
      } else if ((effectiveAdapterResult.exitCode ?? 0) === 0 && !effectiveAdapterResult.errorMessage) {
        outcome = "succeeded";
      } else {
        outcome = "failed";
      }
      ...
      const usageJson =
        normalizedUsage || effectiveAdapterResult.costUsd != null
          ? ({
              ...(normalizedUsage ?? {}),
              ...(rawUsage ? {
              ...
              provider: readNonEmptyString(effectiveAdapterResult.provider) ?? "unknown",
              biller: resolveLedgerBiller(effectiveAdapterResult),
              model: readNonEmptyString(effectiveAdapterResult.model) ?? "unknown",
              ...(effectiveAdapterResult.costUsd != null ? { costUsd: effectiveAdapterResult.costUsd } : {}),
              billingType: normalizeLedgerBillingType(effectiveAdapterResult.billingType),
            } as Record<string, unknown>)
          : null;

      const redactedRunError =
        outcome === "succeeded"
          ? null
          : redactCurrentUserText(
              effectiveAdapterResult.errorMessage ?? (outcome === "timed_out" ? "Timed out" : "Adapter failed"),
              currentUserRedactionOptions,
            );
      const persistedResultJson = enrichHeartbeatRunResultJson(effectiveAdapterResult.resultJson ?? null, {
        summary: effectiveAdapterResult.summary ?? null,
        error: redactedRunError,
        costUsd: effectiveAdapterResult.costUsd ?? null,
      });
        ...
        errorCode:
          outcome === "timed_out"
            ...
            : outcome === "cancelled"
              ? "cancelled"
              : outcome === "failed"
                ? (effectiveAdapterResult.errorCode ?? "adapter_failed")
                : null,
        exitCode: effectiveAdapterResult.exitCode,
        signal: effectiveAdapterResult.signal,
        usageJson,
      ...
      await setWakeupStatus(run.wakeupRequestId, outcome === "succeeded" ? "completed" : status, {
        finishedAt: new Date(),
        error: effectiveAdapterResult.errorMessage ?? null,
      });
      ...
      if (finalizedRun) {
        await updateRuntimeState(agent, finalizedRun, effectiveAdapterResult, {
          legacySessionId: nextSessionState.legacySessionId,
        }, normalizedUsage);
        if (taskKey) {
          if (effectiveAdapterResult.clearSession || (!nextSessionState.params && !nextSessionState.displayId)) {
  ...
  async function releaseIssueExecutionAndPromote(run: typeof heartbeatRuns.$inferSelect) {
    const promotedRun = await db.transaction(async (tx) => {
      ...
      while (true) {
        ...
        const promotedIssueMode = normalizeIssueMode(promotedContextSnapshot.issueMode);
  ...
  async function enqueueWakeup(agentId: string, opts: WakeupOptions = {}) {
    ...
    const wakeIssueMode = issueId
      ? await resolveIssueMode(db, agent.companyId, issueId)
      : normalizeIssueMode(enrichedContextSnapshot.issueMode);
    enrichedContextSnapshot.issueMode = wakeIssueMode;
    const sessionBefore =
        ...
        if (activeExecutionRun) {
          ...
          if (isSameExecutionAgent && !shouldQueueFollowupForCommentWake) {
            ...
            .set({
              ...
              issueMode: normalizeIssueMode(mergedContextSnapshot.issueMode ?? activeExecutionRun.issueMode),
              updatedAt: new Date(),
            })
    ...
    if (coalescedTargetRun) {
      ...
      const mergedRun = await db
        ...
        .set({
          contextSnapshot: mergedContextSnapshot,
          issueMode: normalizeIssueMode(mergedContextSnapshot.issueMode ?? coalescedTargetRun.issueMode),
          updatedAt: new Date(),
        })
