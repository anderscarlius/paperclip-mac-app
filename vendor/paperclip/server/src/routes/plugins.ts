 ...
 *
 * Most routes require board-level authentication. Tool discovery/execution
 * routes also allow authenticated agent callers so heartbeat runs can invoke
...
const BUNDLED_PLUGIN_EXAMPLES: AvailablePluginExample[] = [
  {
    packageName: "@paperclipai/plugin-web-search-example",
    pluginKey: "paperclipai.web-search",
    displayName: "Web Search",
    description: "First-party plugin that exposes a simple public-web search tool for agents.",
    localPath: "packages/plugins/examples/plugin-web-search-example",
    tag: "example",
  },
  {
...
export function pluginRoutes(
  ...
  async function buildPluginHealth(plugin: NonNullable<Awaited<ReturnType<typeof resolvePlugin>>>): Promise<PluginHealthCheckResult> {
    const checks: PluginHealthCheckResult["checks"] = [];
    const workerManager = bridgeDeps?.workerManager ?? webhookDeps?.workerManager ?? null;
    ...
    const workerRunning = workerManager?.isRunning(plugin.id) ?? false;
    checks.push({
      name: "worker_running",
      passed: workerRunning,
      message: workerManager
        ? (workerRunning ? "Worker is running" : "Worker is not running")
        : "Worker manager is not available",
    });

    const declaredTools = plugin.manifestJson?.tools ?? [];
    const registeredToolCount = toolDeps?.toolDispatcher.toolCount(plugin.pluginKey) ?? 0;
    checks.push({
      name: "tools_registered",
      passed: declaredTools.length === 0 || registeredToolCount >= declaredTools.length,
      message: declaredTools.length === 0
        ? "Plugin does not declare agent tools"
        : `${registeredToolCount}/${declaredTools.length} declared tools are registered`,
    });

    let testCallOk = false;
    let testCallMessage = "Health RPC was not run";
    if (workerManager && workerRunning) {
      try {
        const health = await workerManager.call(plugin.id, "health", {}, 5_000);
        testCallOk = health.status === "ok" || health.status === "degraded";
        testCallMessage = health.message ?? `Worker health status: ${health.status}`;
      } catch (error) {
        testCallMessage = error instanceof Error ? error.message : String(error);
      }
    } else if (!workerManager) {
      testCallMessage = "Worker manager is not available";
    } else {
      testCallMessage = "Worker is not running";
    }
    checks.push({
      name: "test_call_ok",
      passed: testCallOk,
    ...
    return {
      ...
      healthy: isReady && hasValidManifest && hasNoError && workerRunning && testCallOk,
      checks,
  ...
  function assertPluginToolRunContextAccess(req: Request, runContext: ToolRunContext) {
    const actor = getActorInfo(req);
    assertCompanyAccess(req, runContext.companyId);
    if (actor.actorType !== "agent") return;

    if (actor.agentId !== runContext.agentId) {
      throw forbidden("Agent key can only execute plugin tools for its own agentId");
    }

    if (actor.runId && actor.runId !== runContext.runId) {
      throw forbidden("Agent key can only execute plugin tools for the current runId");
    }
  }
  ...
  router.post("/plugins/tools/execute", async (req, res) => {
    ...
    assertPluginToolRunContextAccess(req, runContext);
  ...
  router.get("/plugins/:pluginId/health", async (req, res) => {
    ...
    const result = await buildPluginHealth(plugin);
  ...
  router.get("/plugins/:pluginId/dashboard", async (req, res) => {
    ...
    const health = await buildPluginHealth(plugin);
